...
// src/convertValueToTailwind.ts
function convertValueToTailwind(valueType, value) {
  switch (valueType) {
    case "gap":
...
var init_convertValueToTailwind = __esm({
  "src/convertValueToTailwind.ts"() {
    "use strict";
  }
});
...
function getSelectedComponentSetDefaultVariant() {
  const selection = figma.currentPage.selection;
  if (selection.length !== 1) {
    const message = selection.length === 0 ? "Please select a component set." : "Please select only one component set.";
    figma.notify(message, { timeout: 3e3, error: true });
    return null;
  }
